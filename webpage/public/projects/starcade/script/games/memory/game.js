    ...
    for (let copy = 0; copy < 2; copy++) { // force inner loop to loop twice
        for (let i = 2; i < nUniqueCards+2; i++) {
            ...
            if (window.location.pathname == "/php/gamescreen.php") {
                ...
                var flipGameFront = $('<img>').addClass('flipCardFrontGame').attr('src', `/projects/starcade/media/gameimages/memory/${data[0].portrait[2]}`)
                //Create flipcardback with 'front of frame' picture
                var flipGameBack = $('<img>').addClass('flipCardBackGame').attr('src', `/projects/starcade/media/gameimages/memory/${im}`)
            }
